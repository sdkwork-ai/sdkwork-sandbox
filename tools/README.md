...
8. **Zero-requirement claims.** The document asserts in a dozen places that some capability has no requirement behind it. Section 3.4 registers each such claim with the keywords that stand for it, and the gate re-derives ownership from every requirement record's id, slug and title: a keyword that now matches a record turns the row red, naming the record. The claim may only be asserted from the registry, so every such phrasing elsewhere must cite its row (`〔§3.4/N〕`) and every row must be cited at least once -- the same two-way accounting rule 7 of the field gate applies to OpenAPI operations. The reading is deliberately narrow and says so: id, slug and title, not the body, because the bodies mention these words in other senses (`suspended` spawn, a key rotation's `Pause/Resume`, a stream's `resume cursor`) and reading them would redden the whole table for the wrong reason. This is the rule that would have caught the false gap row: register "the performance baseline is unowned" with `allocation` and it fails against `REQ-2026-0006-sandbox-provider-allocation-key-rotation` and `REQ-2026-0019-sandbox-runtime-pool-and-fast-allocation`. The same family holds the cross-document account in section 3.5: the claim phrasing appears in five other documents, the census recounts each declared section so every assertion is counted (the phrasing list itself is a closed set that had to be widened once -- `无独立 REQ-*`, the same assertion with an adjective between determiner and token, was silently missed until the extraction rule was checked against counterexamples -- 19 claims in five documents at the widening, 18 after `REQ-2026-0028` retired the SDK-family claim into the correction ledger), and the attribution ledger judges every counted assertion against the catalog with a closed verdict vocabulary. For each `确认无承载` row the ledger names the keyword standing for the capability and the requirement records that were actually checked, and the gate re-derives the lexical half: a checked candidate that owns the keyword turns the row red, and a requirement record added later that owns some row's keyword turns the ledger red on every future run. Per document, the number of attribution rows must equal the census's declared count, so a claim cannot be added without a verdict or a verdict outlive its claim.
...
Current reading: 17 categories, 78 rows, census `✅ 0 | 🟡 16 | ❌ 60 | ⛔ 2`, 1 residual gap, 7 registered zero-requirement claims, 98 of 98 workspace tests accounted for by section 3.1, 11 component rows and 6 line-numbered anchors resolved by section 1.2, 9 advantage rows and 4 line-numbered anchors resolved by section 5; twelve rule families are declared consistently in all five surfaces that describe this gate.
