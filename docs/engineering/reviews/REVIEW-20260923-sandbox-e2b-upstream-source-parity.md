...
## 14. 追加：§3.2 空档 2 收口——跨文档断言逐条归属判定（2026-09-24）

§3.2 第 2 行登记的空档（「无需求承载」断言只有计数、没有逐条归属对账）本轮闭合。这是 §6 列出的最后一项代理可做工作；此后除 §7 的 ③ 与 C 两个人工决策外，对齐基础设施侧没有已登记的开放项。

### 14.1 句型清单自己的盲区：`无独立 REQ-*` 漏数 4 行

动手判定前先复核计数器，随即抓到第三次同类事故：普查句型要求决定词与 `REQ-*` 之间只有空白，于是 **`无独立 ` + `REQ-*` 这一形态被整类静默漏数**。实测全 `docs/**` 扫描，漏掉的断言有 4 行——PRD.md 第 8 节 MCP 行、PRD-capabilities.md 第 11 节 Auto Pause 行与 MCP 行、PRD-sandbox-surfaces.md 第 1 节 Port Exposure 行。`无独立 REQ-*` 与 `无 REQ-*` 是同一个断言语类，漏数即意味着这些能力从未进入归属对账。这已是同一课的第三次同向验证（`#[tokio::test(...)]` 带参数被丢、operationId 含点被丢、本次的决定词带形容词），已把 `无独立` 并进 `ZERO_REQUIREMENT_PATTERNS` 并在源码注释里记下成因。句型拓宽后：跨文档断言 15 行 → **19 行**、覆盖文档 4 → **5 个**；本文档自己的两行（矩阵第 17、73 行）按第 3.4 节规则补了 `〔§3.4/5〕`、`〔§3.4/7〕` 引用。

### 14.2 归属账：19 行断言逐条判定（§3.5 第三张表）

归属账逐行五要素：文档、能力、关键词、判定（封闭词表：`口径句，不指能力` / `确认无承载` / `已证伪，见更正账`）、已核对候选。19 行的判定结果：

- **15 行 `确认无承载`**（运行模式分层、Template 与构建链、Fork、按需内存与 COW 根文件系统、端口暴露 ×3、`shared` 模式 ×2、Sandbox 内 Agent 运行时、MCP 执行面 ×2、Skills、SDK 家族、Node Drain 与迁移、Port Exposure）——每行点名核对时最可疑的候选记录（共引用 9 份不同的 REQ）；
- **2 行 `口径句，不指能力`**（PRD §8 与 PRD-capabilities §11 的表引言，定义性句子而非能力断言）；
- **1 行 `已证伪，见更正账`**（gate-zero 视图对 Benchmark 断言的历史转述，归到更正账第 1 行）。

判定方法与边界：词法半句（"候选记录不承载该能力"）由门禁按 `requirementOwnsKeyword` 逐（关键词，候选）对复算；语义半句（"目录中确实无人承载"）是逐条人工核对 27 份记录的 id/slug/title 后的判断，随目录增长必须复核——新增记录若拥有某行关键词，第 3.4 节的目录反查会在候选之外独立转红。因此这半张账是**可被目录推翻的活账**，不是一次性结论。逐条核对的显著反例：端口暴露三行都把 `REQ-2026-0023`（内部控制面）列为最可疑候选——内部控制面是端口暴露的必经路径但不是端口暴露能力本身；Auto Pause 把 `REQ-2026-0019/0020` 列为候选——池化与热状态保留都不是超时自动暂停。

### 14.3 门禁新增的判据（family 8，§3.5 归属账）

1. **表存在且非空**、行号连续；判定落在封闭词表。
2. **每文档账数等于普查声明数**——断言增删而判定不同步即转红；归属账点名普查未登记的文档同样转红。
3. **`确认无承载` 行三查**：关键词非空、候选记录可解析、且每对（关键词，候选）词法不命中——命中即说明候选就是承载者，该行判定为假。
4. **`已证伪，见更正账` 行**按能力名与更正账联接（能力全仓皆假，不按文档限联），无对应更正行即转红。
